...
export default function ProductEditForm({ data }: { data?: Product }) {
  const categories = allCategories.map((x) => {
    return { value: x, label: x };
  });

  const title = data ? "Edit Product" : "Create Product";
  const initial_values: ProductFormData = data as ProductFormData;
        ...
        <Form.Item dependencies={["url"]}>
          {() => (
            <Image
              ...
              src={form.getFieldsValue()["image_url"]}
            />
          )}
        </Form.Item>

...
export default function ProductForm({ data }: Props) {
  const categories = allCategories.map((x) => {
    return { value: x, label: x };
  });
  const initial_values: ProductFormData = data ? (data as ProductFormData) : {};
    ...
    <>
      ...
      >
        ...
        <Form.Item label="Product Description" name="description">
          <TextArea showCount maxLength={1000} required />
        </Form.Item>
        ...
        <Form.Item label="In Stock Quantity" name="stock" required>
          <InputNumber min={1} />
        </Form.Item>
        ...
        <Form.Item dependencies={["url"]}>
          {() => (
            <Image
              ...
              src={form.getFieldsValue()["imageURI"]}
            />
          )}
        </Form.Item>
